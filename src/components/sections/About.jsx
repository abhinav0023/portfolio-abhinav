...
const About = ({ about, highlights, highlightsTitle, tags }) => {
  return (
    <section id="about" className="py-14 md:py-24 lg:py-32 px-4 bg-white relative overflow-hidden mb-10 md:mb-0">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 lg:gap-16 items-center">
        <div className="lg:col-span-7">
          <SectionTitle>About Me</SectionTitle>
          <div className="bg-secondary p-6 sm:p-8 md:p-12 border-4 border-black shadow-neo -rotate-1 hover:rotate-0 transition-transform cursor-help">
            ...
            <div className="flex gap-2 sm:gap-4 flex-wrap">
              {tags.map((tag) => (
                <span key={tag} className="bg-white px-3 py-0.5 sm:px-4 sm:py-1 border-2 sm:border-4 border-black font-bold uppercase text-xs sm:text-sm">
                  {tag}
                </span>
              ))}
            </div>
        ...
        <div className="lg:col-span-5 relative h-full flex flex-col justify-center gap-4 md:gap-8">
          {highlightsTitle ? (
            <div className="relative mb-2 inline-block self-start">
              <span className="relative z-10 text-sm sm:text-base md:text-xl font-black uppercase tracking-tight px-2.5 py-1.5 sm:px-3 sm:py-2 bg-white border-2 sm:border-4 border-black shadow-neo-sm -rotate-1">
                {highlightsTitle}
              </span>
            </div>
          ) : null}
